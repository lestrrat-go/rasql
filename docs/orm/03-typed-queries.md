...
This table enumerates the typed layer in `rasql`. [Dynamic rows](../core/05-dynamic.md#operation-reference) has the matching table for `rasql/dynamic`, which names a column as a string instead.
...
The typed builder comes from `SelectFrom`, `DecodeFrom`, and `DecodeFromRef` in `rasql`. It takes a `query.ColumnRef` wherever it names a column, usually a generated accessor such as `users.ID()`, so a wrong name does not compile and a join can order by a column of any table in the statement. [`dynamic.SelectBuilder`](../core/05-dynamic.md#select-builder-methods) is the builder that names its columns as strings instead.
...
which is not valid SQL in any supported dialect. The same two rules hold on
[`dynamic.SelectBuilder`](../core/05-dynamic.md#select-builder-methods).
...
[`dynamic.DeleteFrom(table.Ref()).Returning(...).Query(ctx, db)`](../core/05-dynamic.md#delete-rows)
reads the same rows as `dynamic.Row` values.
...
`query.And` and `query.Or` take expressions and return one, so either holds the other and a filter that mixes them is a single tree passed to a single `Where` call. Nothing limits the depth, and every constructor in [Where conditions](../core/02-sql-builder.md#where-conditions) can sit at any node.
...
`query.InSelect` and `query.Scalar` take a `SELECT` statement as the right-hand side of a predicate, in place of a value list or a bound value. Each subquery is validated
as part of the statement that encloses it and rendered inside it. [Subqueries](../core/02-sql-builder.md#subqueries) covers the placement rules, including MySQL's
restriction on `LIMIT` inside an `InSelect` statement.

Neither subquery below reads the enclosing statement's row: the first names another table, and the second aliases `orders` so its own scope is separate from the `orders`
the enclosing statement reads. [Filter with EXISTS](#filter-with-exists) is the correlated form.
...
`query.Exists(statement)` and `query.NotExists(statement)` test whether a subquery returns any row. They read no value from it, so the statement may project anything;
project a column of its own table.

Call `query.Select.WithCorrelation(users.Ref())` on the subquery before the `WithWhere` that reads a `users` column, so the subquery says which enclosing table it reads
before it reads one. The database then evaluates the subquery once per enclosing row, and `EXISTS` asks whether that particular user has an order rather than whether the
...
[`dynamic.SelectBuilder`](../core/05-dynamic.md#select-builder-methods) groups by a primary-table column name with `GroupByColumns` instead, without importing the `query` package.
...
[Where conditions](../core/02-sql-builder.md#where-conditions) lists every comparison, logical connective, and null test the expression set offers.
