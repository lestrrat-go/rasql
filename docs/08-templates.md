...
`Compiled.GoSource` emits a Go function that builds the statement, so a template can be compiled at build time rather than at startup. Put the template in the `queries` list of `rasql.json`; that keeps table generation and static query generation in one command. See [`rasql codegen`](09-rasqlgen.md#static-query-functions).

## Next

[`rasqlgen`](09-rasqlgen.md) generates table descriptors and query functions as Go source.
