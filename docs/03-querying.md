...
Columns come from the generated table value: `users.ID` is a `query.Column` bound to the `users` table, so a misspelled column name is a compile error rather than a database one.
