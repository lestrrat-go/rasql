...
Supply exactly one of `-dsn` or `-source`. Direct inspection supports PostgreSQL, MySQL, and SQLite; the command bundles their `pgx`, `mysql`, and `sqlite` drivers, so nothing needs importing to use `-dsn`. PostgreSQL inspection preserves supported columns (recording a generated column's expression and storage kind in `ColumnDef.GeneratedExpression` and `.GeneratedStorage`, reading the storage kind from `pg_catalog.pg_attribute.attgenerated` since `information_schema` itself states only that a column is generated), primary keys, named unique constraints (recording a deferrable constraint, a `NULLS NOT DISTINCT` constraint, and `INCLUDE` columns in `UniqueDef.Deferrable`, `.NullsNotDistinct`, and `.IncludeColumns`), checks (recording `NO INHERIT`, `NOT VALID`, and `NOT ENFORCED` in `CheckDef.NoInherit`, `.NotValid`, and `.NotEnforced`), indexes (recording a non-default access method such as `gin` in `IndexDef.Method`, generated as `schema.IndexMethod("gin")`, a partial index's `WHERE` predicate in `IndexDef.Predicate`, an expression index's key text in `IndexDef.Expressions`, an `INCLUDE` clause's columns in `IndexDef.IncludeColumns`, and a `DESC` key or a non-default per-key collation or operator class in `IndexDef.Keys`), foreign keys (recording a reference into another schema, a non-default `MATCH` clause, a deferrable constraint, and `NOT VALID`/`NOT ENFORCED` in `ForeignKeyDef.ReferencedSchema`, `.Match`, `.Deferrable`, `.NotValid`, and `.NotEnforced`), and exclusion constraints (recording each `EXCLUDE` constraint as a `schema.ExclusionDef`, pairing each element's expression with its operator, and recording a non-default access method, a `WHERE` predicate, and a deferrable constraint when the constraint states them), including exact decimal columns (`NUMERIC`/`DECIMAL`), which generate a Go `string` field and whose generated descriptor restates the column's `Precision` and `Scale`. MySQL inspection preserves supported columns (recording a stated display width and `ZEROFILL` in `IntegerType.DisplayWidth` and `.ZeroFill`, and a generated column's expression and storage kind, read from `EXTRA` and `GENERATION_EXPRESSION`, in `ColumnDef.GeneratedExpression` and `.GeneratedStorage`), primary keys, indexes, likewise recording a non-default index type such as `FULLTEXT`, a functional index's key text in `IndexDef.Expressions`, a `DESC` or column-prefix key part in `IndexDef.Keys`, and an `INVISIBLE` index in `IndexDef.Invisible`, and checks (recording `NOT ENFORCED` in `CheckDef.NotEnforced`; MySQL has no `NO INHERIT` or `NOT VALID` concept for check constraints, so neither ever comes from a MySQL descriptor); SQLite inspection preserves supported columns, including a generated column's expression and storage kind in `ColumnDef.GeneratedExpression` and `.GeneratedStorage`, and primary keys (recording a primary key's `AUTOINCREMENT` keyword and `ON CONFLICT` resolution in `TableDef.PrimaryKeyAutoincrement` and `.PrimaryKeyOnConflict`), a `STRICT` or `WITHOUT ROWID` table option in `TableDef.Strict` and `.WithoutRowID`, and unique constraints' `ON CONFLICT` resolution in `UniqueDef.OnConflict`. <!-- MySQL includes indexes here; SQLite does not, and no later clause contradicts that distinction. --> A generated `schema.Decimal` call restates precision and scale positionally, and states scale even when it is `0`, because the zero value of `schema.DecimalScale` means no scale was stated and `TableDef.Validate` rejects a decimal column that states none. It reports an error when an inspected type or constraint has metadata that the schema descriptor cannot reproduce, when a unique constraint has metadata beyond deferrability, `NULLS NOT DISTINCT`, and `INCLUDE` columns that the descriptor cannot reproduce (a temporal unique constraint, or one whose backing index uses a nondefault collation, storage option, tablespace, or replica identity, for example), when an index has metadata beyond its method, predicate, key expressions, `INCLUDE` columns, and per-key facts that the descriptor cannot reproduce (an invalid index, or one with a nondefault storage option, tablespace, or replica identity, for example), when a foreign key has metadata beyond referenced schema, match type, deferrability, validation, and enforcement that the descriptor cannot reproduce (a temporal foreign key, for example), when a SQLite table is a virtual table, a `CREATE TABLE AS SELECT` definition, or an otherwise unsupported table kind, when a SQLite column is genuinely hidden rather than generated, the kind a virtual table module declares, and when a PostgreSQL column is a bare, unconstrained `NUMERIC`, since PostgreSQL reports no precision for it to record. A non-default index method, a partial index's predicate, and an expression index's key text are all describable, not yet renderable: see [Index methods](02-schema.md#index-methods) and [Partial and expression indexes](02-schema.md#partial-and-expression-indexes). The same is true of an index's `IncludeColumns`, `Invisible` marker, and per-key `Keys` — a `DESC` order, a non-default collation or operator class, and a MySQL prefix length: see [Index key details](02-schema.md#index-key-details). And of a non-default foreign key `Match` or `Deferrable`, a `NOT VALID` or `NOT ENFORCED` check constraint or foreign key, a `NO INHERIT` check constraint, a unique constraint's `Deferrable`, `NullsNotDistinct`, `IncludeColumns`, or `OnConflict`, a table's `Strict`, `WithoutRowID`, `PrimaryKeyAutoincrement`, or `PrimaryKeyOnConflict`, a stated integer display width or `ZEROFILL`, a generated column, and any exclusion constraint at all: see [Foreign key facts](02-schema.md#foreign-key-facts), [Check constraint facts](02-schema.md#check-constraint-facts), [Unique constraint facts](02-schema.md#unique-constraint-facts), [SQLite table-level options](02-schema.md#sqlite-table-level-options), [Integer display width and ZEROFILL](02-schema.md#integer-display-width-and-zerofill), [Generated columns](02-schema.md#generated-columns), and [Exclusion constraints](02-schema.md#exclusion-constraints).
