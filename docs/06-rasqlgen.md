...
For a `users` table the file declares a row type and a table accessor:
...
`store.Users()` returns a `rasql.Table[store.UsersRow]`, ready for `rasql.SelectFrom`, `rasql.Insert`, and `rasql.Update`. `store.Users().Ref()` gives the `query.TableRef` the lower-level API takes.

The descriptor is a package-level variable, which Go cannot mark constant, so it stays unexported and only the accessor is exported. Importing code therefore cannot swap the descriptor out from under the rest of the program, and each call returns a copy of a value whose only field is unexported.
