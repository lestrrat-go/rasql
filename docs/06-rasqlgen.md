...
A refresh only ever touches a directory `bootstrap` recognizes as its own: every `_gen.go` and `_gen_test.go` file must open with `// Code generated by rasqlgen bootstrap; DO NOT EDIT.`, and `hints.go` is the one exception allowed without it. A directory holding anything else -- a hand-written file, output from another generator, or output from `rasqlgen schema` or `rasqlgen query` -- is refused the same way a first run into a non-empty directory is.
