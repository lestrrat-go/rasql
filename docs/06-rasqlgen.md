...
A generated column changes nothing about the row type either: `rasqlgen` still generates an ordinary field for it, of the same Go type its logical type would otherwise produce, since a generated column is read back like any other column and only writing to it is restricted. The generated descriptor literal restates `GeneratedExpression` and `GeneratedStorage`, so regenerating keeps the column generated instead of turning it into an ordinary writable one. A caller that needs to omit a generated column from an `INSERT` or `UPDATE` uses `rasql.DefaultColumns` or `rasql.UpdateColumns`, the same options that already omit a database-default or auto-increment column. See [Generated columns](02-schema.md#generated-columns).
