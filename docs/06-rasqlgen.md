...
| `-output` | Directory for generated files, created (including any missing parents) if it does not already exist: empty for a first run, or a directory `bootstrap` already wrote to for a refresh. Required. |
...
`-output` is created, including any missing parents, when it does not already exist. Bootstrapping into an empty `-output` writes a new package, as shown above. Run again against a directory `bootstrap` already wrote to, `-output` is treated as a refresh instead of a second write, described next. Any other non-empty `-output` -- a directory holding a hand-written file, or any file `bootstrap` did not itself write -- is refused exactly as before: `bootstrap` only ever writes into a directory it started, or one it already owns.
...
| `-table` | Table to generate instead of sweeping every base table with `-dsn`; repeat it for each table. Filters a package from `-source` the same way. Not accepted together with `-exclude`. Passing the same table name twice is an error. |
| `-exclude` | Table to skip during a `-dsn` sweep; repeat it for each table. Not accepted together with `-table`, and not supported with `-source`. Passing the same table name twice is an error. |
| `-dialect` | Dialect and driver for `-dsn`, defaulting to `postgresql`. |
...
### What inspection preserves

#### PostgreSQL

PostgreSQL preserves supported columns, primary keys, named unique constraints, checks, indexes, foreign keys, and exclusion constraints:

- A generated column's expression and storage kind, in `ColumnDef.GeneratedExpression` and `.GeneratedStorage`. The storage kind is read from `pg_catalog.pg_attribute.attgenerated`, since `information_schema` itself states only that a column is generated.
- An exact decimal column (`NUMERIC`/`DECIMAL`), which generates a Go `string` field and whose generated descriptor restates the column's `Precision` and `Scale`.
- A named unique constraint's deferrability, `NULLS NOT DISTINCT`, `INCLUDE` columns, and `WITHOUT OVERLAPS` (temporal) declaration, and its backing index's storage parameters, tablespace, replica identity, and per-column collations, in `UniqueDef.Deferrable`, `.NullsNotDistinct`, `.IncludeColumns`, `.Temporal`, `.StorageParameters`, `.Tablespace`, `.ReplicaIdentity`, and `.Collations`.
- A check constraint's `NO INHERIT`, `NOT VALID`, and `NOT ENFORCED`, in `CheckDef.NoInherit`, `.NotValid`, and `.NotEnforced`.
- An index's non-default access method such as `gin` in `IndexDef.Method`, generated as `schema.IndexMethod("gin")`; a partial index's `WHERE` predicate in `.Predicate`; an expression index's key text in `.Expressions`; an `INCLUDE` clause's columns in `.IncludeColumns`; a `DESC` key, a non-default per-key collation or operator class, or a `NULLS FIRST`/`NULLS LAST` override in `.Keys`; an invalid index, its storage parameters, its tablespace, and whether it is the table's replica identity in `.NotValid`, `.StorageParameters`, `.Tablespace`, and `.ReplicaIdentity`; and a plain unique index's `NULLS NOT DISTINCT` in `.NullsNotDistinct`.
- A foreign key's reference into another schema, a non-default `MATCH` clause, a deferrable constraint, `NOT VALID`/`NOT ENFORCED`, a `PERIOD` (temporal) foreign key, and an `ON DELETE SET NULL`/`SET DEFAULT` column list, in `ForeignKeyDef.ReferencedSchema`, `.Match`, `.Deferrable`, `.NotValid`, `.NotEnforced`, `.Temporal`, and `.DeleteSetColumns`.
- Each `EXCLUDE` constraint as a `schema.ExclusionDef`, pairing each element's expression with its operator, and recording a non-default access method, a `WHERE` predicate, and a deferrable constraint when the constraint states them.

#### MySQL

MySQL preserves supported columns, primary keys, indexes, and checks:

- A stated display width and `ZEROFILL`, in `IntegerType.DisplayWidth` and `.ZeroFill`.
- `UNSIGNED` and `ZEROFILL` on a decimal column, in `DecimalType.Unsigned` and `.ZeroFill`.
- A generated column's expression and storage kind, read from `EXTRA` and `GENERATION_EXPRESSION`, in `ColumnDef.GeneratedExpression` and `.GeneratedStorage`.
- A non-default index type such as `FULLTEXT` in `IndexDef.Method`, a functional index's key text in `.Expressions`, a `DESC` or column-prefix key part in `.Keys`, and an `INVISIBLE` index in `.Invisible`.
- A check constraint's `NOT ENFORCED`, in `CheckDef.NotEnforced`. MySQL has no `NO INHERIT` or `NOT VALID` concept for check constraints, so neither ever comes from a MySQL descriptor.

#### SQLite

SQLite preserves supported columns, primary keys, unique constraints, checks, foreign keys, and indexes:

- A generated column's expression and storage kind, in `ColumnDef.GeneratedExpression` and `.GeneratedStorage`.
- A virtual table module's own hidden column, in `ColumnDef.Hidden`.
- A primary key's `AUTOINCREMENT` keyword and `ON CONFLICT` resolution, in `TableDef.PrimaryKeyAutoincrement` and `.PrimaryKeyOnConflict`.
- A `STRICT` or `WITHOUT ROWID` table option, in `TableDef.Strict` and `.WithoutRowID`.
- A `CREATE VIRTUAL TABLE` definition's own module name and arguments, in `TableDef.VirtualTableModule` and `.VirtualTableModuleArguments`.
- A unique constraint's `ON CONFLICT` resolution in `UniqueDef.OnConflict`, and a per-key `DESC` order or non-default collation in `UniqueDef.Keys`.
- Checks and foreign keys.
- Every index a `CREATE INDEX` statement created, with its `Unique` flag, its keys in `IndexDef.Columns`, `.Expressions`, or `.Keys` — the last recording a `DESC` order or a non-default collation — and a partial index's `WHERE` predicate in `.Predicate`; no other `IndexDef` field ever comes from SQLite.

#### What inspection refuses

Inspection reports an error instead of a descriptor when an inspected type or constraint has metadata the schema descriptor cannot reproduce:

- A PostgreSQL column of a type no dialect mapping covers, such as an array, enum, domain, `INTERVAL`, `INET`, `MONEY`, or `XML` column, and a PostgreSQL decimal column the catalog reports without a precision or without a scale — including a bare, unconstrained `NUMERIC`, since PostgreSQL reports no precision for it to record. No other PostgreSQL fact about an index, unique constraint, foreign key, check constraint, or exclusion constraint causes inspection to reject a table anymore (see [Read a table out of a database](02-schema.md#read-a-table-out-of-a-database)).
- A MySQL column of a type no dialect mapping covers, such as `BIT`, `YEAR`, `VECTOR`, or a spatial type. No other MySQL modifier or per-key index fact causes inspection to reject a table anymore: the handful of defensive checks still left in `inspect` for a MySQL index key part with neither a column name nor an expression, a `CHAR`/`VARCHAR` declaration carrying a trailing modifier, and an integer or decimal declaration carrying a modifier other than a display width, `UNSIGNED`, or `ZEROFILL`, guard shapes MySQL's own catalog does not produce, not real facts a live server can state.
- A SQLite unique constraint that has an expression key.
- A SQLite table that is a `CREATE TABLE AS SELECT` definition, or a table kind other than a plain table, a virtual table, or a virtual table's own shadow table.

#### Describable but not yet renderable

Each fact below is describable — inspection records it and the generated descriptor restates it — but not yet renderable:

- A non-default index method, a partial index's predicate, and an expression index's key text: see [Index methods](02-schema.md#index-methods) and [Partial and expression indexes](02-schema.md#partial-and-expression-indexes).
- An index's `IncludeColumns`, `Invisible` marker, and per-key `Keys` — a `DESC` order, a non-default collation or operator class, a MySQL prefix length, and a `NULLS FIRST`/`NULLS LAST` override: see [Index key details](02-schema.md#index-key-details).
- An index's `NotValid`, `StorageParameters`, `Tablespace`, `ReplicaIdentity`, and `NullsNotDistinct`: see [Index validity, storage, and placement facts](02-schema.md#index-validity-storage-and-placement-facts).
- A non-default foreign key `Match` or `Deferrable`, a `NOT VALID` or `NOT ENFORCED` foreign key, a temporal foreign key, and an `ON DELETE SET NULL`/`SET DEFAULT` column list: see [Foreign key facts](02-schema.md#foreign-key-facts).
- A `NO INHERIT`, `NOT VALID`, or `NOT ENFORCED` check constraint: see [Check constraint facts](02-schema.md#check-constraint-facts).
- A unique constraint's `Deferrable`, `NullsNotDistinct`, `IncludeColumns`, `OnConflict`, `Keys`, `Temporal`, `StorageParameters`, `Tablespace`, `ReplicaIdentity`, or `Collations`: see [Unique constraint facts](02-schema.md#unique-constraint-facts).
- A table's `Strict`, `WithoutRowID`, `PrimaryKeyAutoincrement`, or `PrimaryKeyOnConflict`: see [SQLite table-level options](02-schema.md#sqlite-table-level-options).
- A table's `VirtualTableModule`: see [SQLite virtual tables](02-schema.md#sqlite-virtual-tables).
- A stated integer display width or `ZEROFILL`: see [Integer display width and ZEROFILL](02-schema.md#integer-display-width-and-zerofill).
- A decimal column's `UNSIGNED` or `ZEROFILL`: see [Decimal UNSIGNED and ZEROFILL](02-schema.md#decimal-unsigned-and-zerofill).
- A generated column: see [Generated columns](02-schema.md#generated-columns).
- Any exclusion constraint: see [Exclusion constraints](02-schema.md#exclusion-constraints).
...
A generated `schema.Decimal` call restates precision and scale positionally, and states scale even when it is `0`, because the zero value of `schema.DecimalScale` means no scale was stated and `TableDef.Validate` rejects a decimal column that states none.
...
The generator owns the migration setup, database path, table selection, and generated-store policy. The separate Taskboard module is checked by root tests, but its generated files are refreshed from that module with the commands above.
...
`rasqlgen` never writes a generated file in place. It writes a temporary file beside each table or query file and renames it over the destination only after the write succeeds, so a failed run leaves an existing file untouched. Existing files keep their permission bits and sticky bit. On Unix platforms that replacement is atomic. A `schema` or `bootstrap` output directory no longer has to already exist; it is created, including any missing parents, the same as `mkdir -p` -- a symbolic link to a directory is also allowed and left as-is. A `query` output path is unaffected: `-output` for `query` names a single file, not a directory, and its parent directory still has to exist.
