...
| `-output` | Existing, empty directory for generated files. Required. |
...
Every other file `bootstrap` writes is inspected from the database and freely regenerated; `hints.go` is the opposite, since a row type name is a fact no database has an opinion about. `bootstrap` writes it exactly once, the first time it creates the package, and never touches it again afterward -- so an entry added under `"users"` there, such as `RowName: "User"`, survives every later `rasqlgen bootstrap` run against the same `-output` directory. `tables_gen.go`'s own `Tables` function, shown above, is what applies every entry in `Hints` before returning, which is why it would be rewritten on a later run while `Hints` itself never is.

`-output` must already exist and be empty. `bootstrap` refuses a directory holding any files at all, hand-written or its own earlier output alike: regenerating an already-bootstrapped package is not supported yet, and is left for a follow-up.
