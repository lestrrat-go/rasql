...
```go
// Code generated by rasqlgen bootstrap once; edit freely.
...
A refresh only ever touches a directory `bootstrap` recognizes as its own: every `_gen.go` and `_gen_test.go` file must open with `// Code generated by rasqlgen bootstrap; DO NOT EDIT.`, or still with the single `// Code generated by rasqlgen; DO NOT EDIT.` every command wrote before this split -- a package `bootstrap` generated before the split is recognized too, and its files are adopted as this refresh writes them -- and `hints.go` is the one exception allowed without either. A directory holding anything else -- a hand-written file, output from another generator, or output from `rasqlgen schema` or `rasqlgen query` -- is refused the same way a first run into a non-empty directory is.
...
`rasqlgen` replaces only files it wrote itself. `bootstrap`, `schema`, and `query` each write their own marker naming the command: `// Code generated by rasqlgen bootstrap; DO NOT EDIT.`, `// Code generated by rasqlgen schema; DO NOT EDIT.`, and `// Code generated by rasqlgen query; DO NOT EDIT.`. A destination that already exists must open with the writing command's own marker; a destination opening with a different rasqlgen command's marker, or with anything else, stops the run with an error naming both the file and the command that actually wrote it, so a hand-written file, or output from a different `rasqlgen` command, that happens to sit where generated output lands is never destroyed. A `schema` run checks every file it is about to write before it writes the first one, so that refusal cannot leave a package with some files regenerated and the rest as they were. Regenerating output from an earlier run of the same command is unaffected, since that output already carries the matching marker. A file generated before this marker split still opens with the single `// Code generated by rasqlgen; DO NOT EDIT.` every command used to write; that older mark is not refused, it is adopted -- the first command to write over such a file accepts it and rewrites that first line to its own marker, which is the upgrade path for a package an earlier `rasqlgen` produced. From that write on, the file is held to the ordinary same-command check like any other.
