...
Inspecting a live MySQL database preserves the width a `CHAR(n)` or `VARCHAR(n)` column states, so a column created with `schema.Width` round-trips through `inspect.Inspector` unchanged. `TEXT`, `ENUM`, and `SET` columns normalize to an unstated width, since none of them carries a plain numeric length in MySQL's catalog.
