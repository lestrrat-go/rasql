...
`DecimalType.Unsigned` and `DecimalType.ZeroFill` record the same two MySQL-only attributes on a `DECIMAL` or `NUMERIC` column that `IntegerType.Unsigned` and `.ZeroFill` record for an integer column (see [Integer display width and ZEROFILL](#integer-display-width-and-zerofill)): `Unsigned` narrows the column to non-negative values, and `ZeroFill` left-pads a displayed value with zeros and implies `Unsigned`. `inspect.Table` now records both instead of failing the whole table, as it used to whenever a MySQL decimal column carried either. Unlike `IntegerType.Unsigned`, which every dialect that has an unsigned integer type can render, `DecimalType.Unsigned` has no renderable form at all yet: MySQL itself has deprecated the attribute since 8.0.17, and no dialect's `decimalTypeName` builds one.

Both are plain `bool` fields; their zero value, `false`, means the column carries neither, which is what every `DecimalType` and every checked-in generated file written before these fields existed has always meant.

Both facts are describable but not yet renderable. `render.CreateTable` refuses to build DDL for a decimal column stating either, returning a `*render.UnsupportedDecimalUnsignedError` or `*render.UnsupportedDecimalZeroFillError` that names the column and the fact it named, checking `ZeroFill` first since it always implies `Unsigned`; the migrate diff-live path inherits the same refusal because it builds its SQL through `render.CreateTable`. There is no option-form constructor for either yet: today they only appear on a descriptor `inspect` produced. PostgreSQL and SQLite have neither concept, so neither field ever comes from a PostgreSQL or SQLite descriptor.
