...
PostgreSQL inspection still refuses an invalid index, one with a nondefault persistent storage option, tablespace, or replica identity, or one whose `NULLS NOT DISTINCT` setting rasql cannot represent outside a named unique constraint; those stay out of scope for now. A PostgreSQL key's `NULLS FIRST`/`NULLS LAST` placement is refused too whenever it does not match the default that its own `ASC`/`DESC` direction already implies — rasql models `Descending` but not an independent nulls-placement fact.
...
`ColumnDef.GeneratedExpression` and `ColumnDef.GeneratedStorage` describe a generated column: one whose value SQLite computes from an expression over other columns, rather than one an `INSERT` or `UPDATE` can write to directly. `inspect.Table` now records both instead of failing the whole table, as it used to whenever a table carried a hidden or generated column; a genuinely hidden column, the kind a virtual table module declares, is still refused.
...
This release covers SQLite only. PostgreSQL and MySQL both have generated columns too, but inspecting one there remains out of scope for now; a table with a generated column on either engine is re-read through a hand-written descriptor, the same way a qualified PostgreSQL or MySQL table is (see [Qualify a table with a schema](#qualify-a-table-with-a-schema)).
