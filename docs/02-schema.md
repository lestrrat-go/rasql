...
`UniqueDef` carries five facts a live database can attach to a unique constraint beyond a plain column list: `Deferrable`, `NullsNotDistinct`, `IncludeColumns`, `OnConflict`, and `Keys`. `inspect.Table` now records all five instead of failing the whole table, as it used to whenever a PostgreSQL unique constraint was deferrable, used `NULLS NOT DISTINCT`, or named an `INCLUDE` clause, or a SQLite `UNIQUE` constraint named an `ON CONFLICT` clause, a `DESC` key, or a non-default per-key collation.
...
`Keys`, when non-empty, is the full ordered list of per-key facts for a SQLite `UNIQUE` constraint that orders at least one key `DESC` or names a non-default collation on it, reusing `IndexKeyDef` — the same type `IndexDef.Keys` uses (see [Index key details](#index-key-details)) — rather than a second type with the same shape. When `Keys` is set it replaces `Columns` as the source of the constraint's key order, the same way `IndexDef.Keys` replaces `IndexDef.Columns`. `IndexKeyDef.Expression` is always a bare column name here, never expression text: SQLite's own grammar prohibits an expression inside a `UNIQUE` table constraint (only a column reference, optionally `COLLATE`d and ordered, is allowed), so `inspect.Table` still refuses a `UNIQUE` constraint if it ever encounters one anyway, on the theory that a hand-supplied definition might reach rasql's parser even though a live SQLite catalog itself never produces one. `IndexKeyDef.OperatorClass` and `.PrefixLength` never come from a SQLite descriptor, the same as they never come from a SQLite `IndexDef.Keys`. Its zero value, `nil`, means every key is a plain ascending column in the default collation, described by `Columns` exactly as before this field existed. `TableDef.Validate` rejects a `UniqueDef` that sets both `Columns` and `Keys`.

All five are describable but not yet renderable. `render.CreateTable` and the migrate diff-live path refuse to build DDL for a unique constraint naming any of them, returning a `*render.UnsupportedUniqueDeferrabilityError`, `*render.UnsupportedUniqueNullsNotDistinctError`, `*render.UnsupportedUniqueIncludeColumnsError`, `*render.UnsupportedUniqueConflictResolutionError`, or `*render.UnsupportedUniqueKeyDetailsError` that names the constraint and the fact it named, since rasql does not yet know how to construct anything other than a plain `UNIQUE (...)` constraint. There is no option-form constructor for any of the five yet: today they only appear on a descriptor `inspect` produced.

PostgreSQL inspection still refuses a temporal unique constraint and one whose backing index uses a nondefault collation, storage option, tablespace, or replica identity; those stay out of scope for now.
...
`Keys`, when non-empty, is the full ordered list of per-key facts for an index that has at least one key ordered `DESC`, using a non-default collation or operator class, or (MySQL) indexed over a column prefix. These four facts are positional, one per key, so none of them fits `Columns` or `Expressions`, both flat lists with no room to attach a second fact to one position. `Keys` replaces both as the sole source of the index's key order when set, the same way `Expressions` already replaces `Columns` for a mixed or all-expression index: `Columns` and `Expressions` are both left empty, and each `IndexKeyDef.Expression` carries that key's own text — a bare column name for a plain-column key, or expression text otherwise — exactly what `Expressions`'s elements already mean. `TableDef.Validate` rejects an `IndexDef` that sets `Keys` alongside `Columns` or `Expressions`. `IndexKeyDef`'s other fields, `Descending`, `Collation`, `OperatorClass`, and `PrefixLength`, each keep their own zero value — `false`, `""`, `""`, and `0` — for the ordinary case: ascending, default collation, default operator class, whole column. An index with no such key still describes its keys with `Columns` or `Expressions` alone, exactly as before `Keys` existed, so every descriptor and checked-in generated file written before this type existed stays valid.

`OperatorClass` is PostgreSQL-only: MySQL and SQLite have no operator class concept, so it never comes from a MySQL or SQLite descriptor. `PrefixLength` is MySQL-only: PostgreSQL and SQLite have no index-key-prefix concept, so it never comes from a PostgreSQL or SQLite descriptor. `Collation` comes from PostgreSQL or SQLite; MySQL has no per-key collation concept — its own `COLLATION` catalog column names sort order (`A`/`D`), which `Descending` already captures, not a collating sequence.

All three facts are describable but not yet renderable. `render.CreateIndexes` and the migrate diff-live path still refuse to build DDL for any of them: a non-empty `IncludeColumns` returns a `*render.UnsupportedIndexIncludeColumnsError`, a true `Invisible` returns a `*render.UnsupportedIndexInvisibleError`, and a non-empty `Keys` returns a `*render.UnsupportedIndexKeyDetailsError`, each naming the index and, where relevant, what it carries, since rasql does not yet know how to construct an `INCLUDE` clause, an `INVISIBLE` index, a `DESC` key, a non-default collation or operator class, or a MySQL prefix part. None of the three has an option-form constructor yet: today they only appear on a descriptor `inspect` produced.

## Index validity, storage, and placement facts

`IndexDef` carries four more facts a live PostgreSQL index can report about its own state and physical placement, beyond its key order, method, predicate, expressions, and per-key details: `NotValid`, `StorageParameters`, `Tablespace`, and `ReplicaIdentity`. `inspect.Table` now records all four instead of failing the whole table, as it used to whenever a PostgreSQL index was left invalid by a failed concurrent build, carried a storage parameter, lived outside the default tablespace, or served as the table's replica identity — the last of the PostgreSQL index conditions that used to abort a sweep over a production schema outright.
...
All four facts are PostgreSQL-only: MySQL and SQLite have no equivalent to any of them, so none ever comes from a MySQL or SQLite descriptor. All four are describable but not yet renderable. `render.CreateIndexes` and the migrate diff-live path still refuse to build DDL for any of them: `NotValid` returns a `*render.UnsupportedIndexNotValidError`, a non-empty `StorageParameters` returns a `*render.UnsupportedIndexStorageParametersError`, a non-empty `Tablespace` returns a `*render.UnsupportedIndexTablespaceError`, and `ReplicaIdentity` returns a `*render.UnsupportedIndexReplicaIdentityError`, each naming the index and, where relevant, what it carries, since rasql does not yet know how to construct an invalid index, a `WITH (...)` storage clause, a `TABLESPACE` clause, or a `REPLICA IDENTITY USING INDEX` declaration. None of the four has an option-form constructor yet: today they only appear on a descriptor `inspect` produced.

PostgreSQL inspection still refuses two narrower index conditions this change leaves out of scope. A plain (non-constraint) unique index declared `NULLS NOT DISTINCT` is refused, since `IndexDef` has no field for that fact outside a named unique constraint — `UniqueDef.NullsNotDistinct` (see [Unique constraint facts](#unique-constraint-facts)) covers only the constraint form. A key's `NULLS FIRST`/`NULLS LAST` placement is refused too whenever it does not match the default that its own `ASC`/`DESC` direction already implies, since `IndexKeyDef` models `Descending` but not an independent nulls-placement fact. A named `UNIQUE` constraint whose own backing index carries a non-default collation, storage parameters, tablespace, or replica identity is refused on the same terms as before this change, since `UniqueDef` records none of those facts about its backing index.
