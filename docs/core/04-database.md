...
[The SQL builder](02-sql-builder.md) ends at a `stmt.Statement`, which holds SQL text and its arguments. `database/sql` runs one directly. Running it through a `rasql.DB` instead applies the registered hooks and decodes the rows:
...
func Example_rasql_transaction() {
	...
	inTx, err := rasql.SelectFrom(users).OrderAsc(users.ID().Ref()).All(ctx, tx)
	if err != nil {
	...
	// SQL: SELECT users.id, users.email FROM users ORDER BY users.id ASC
	afterCommit, err := rasql.SelectFrom(users).OrderAsc(users.ID().Ref()).All(ctx, db)
	if err != nil {
