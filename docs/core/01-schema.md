...
A column also carries `Nullable`, `Default`, and its concrete `Type`. Type-specific options live on that type: `IntegerType.Unsigned` describes unsigned integers, `IntegerType.DisplayWidth` and `.ZeroFill` describe two MySQL-only integer modifiers (see [Integer display width and ZEROFILL](#integer-display-width-and-zerofill)), `TextType.Width` describes a text column's maximum number of characters, `DecimalType` carries `Precision` and `Scale`, and `DecimalType.Unsigned` and `.ZeroFill` describe the same two MySQL-only modifiers for a decimal column (see [Decimal UNSIGNED and ZEROFILL](#decimal-unsigned-and-zerofill)). `ColumnDef.GeneratedExpression` and `.GeneratedStorage` describe a generated column (see [Generated columns](08-inspection-facts.md#generated-columns)). Identifiers must be simple: `schema.ValidateIdentifier` accepts a leading letter or underscore followed by letters, digits, or underscores, and everything else is rejected rather than quoted around.
