...
// pageFingerprintFields projects what internal/pagekey records about each key.
func pageFingerprintFields[R any](keys []*pageKey[R]) []pagekey.Field {
	fields := make([]pagekey.Field, len(keys))
	for i, key := range keys {
		fields[i] = pagekey.Field{
			Direction: byte(key.direction),
			Nulls:     byte(key.term.nulls),
			Codec:     key.codec,
...
func pageFingerprint[R any](engine, sqlText string, statementSchema ResultSchema, statement stmt.Statement, keys []*pageKey[R], indexes []int) ([32]byte, error) {
	return pagekey.Fingerprint(engine, sqlText, statementSchema.Columns(), statement, pageFingerprintFields(keys), indexes)
}
