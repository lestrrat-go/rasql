...
func (s Select) ValidateCompilerExpression(expression Expression, clause string) error {
	copy := s.clone()
	switch clause {
	case "projection":
		copy.projections = []Projection{Project(expression)}
		copy.where = nil
		copy.having = nil
		copy.orderBy = nil
	case "where":
		copy.where = expression
	case "group":
		copy.groupBy = []Expression{expression}
	case "having":
		copy.having = expression
	case "order":
		copy.orderBy = []Order{Asc(expression)}
	default:
