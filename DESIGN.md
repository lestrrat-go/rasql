...
The migration package applies ordered native SQL migrations for PostgreSQL, MySQL, and SQLite, and reverts them. It keeps an ID and checksum history, preserves source order, and rejects a changed or skipped migration. Every migration carries the reverse sources that undo it, and a revert runs them newest first and deletes each history record; the checksum covers the forward sources alone, so a reverse source can be corrected for a migration that is already applied. Its optional desired-schema diff adapters parse and render dialect-specific SQL to generate reviewed migrations. It records a migration only once every one of its sources has succeeded, so a migration that fails part way through stays pending and runs again from its first source. On MySQL, where most DDL has no idempotent spelling, it reads the six server errors that mean a statement's work was already done as success and warns about each one, so that retry gets past the sources that already succeeded. The runner does not parse migration SQL, automatically repair a live schema, or synthesize migrations from a live schema difference.
...
`rasql.Table[T]` is a struct holding one `query.TableRef` in an unexported field, so nothing outside `rasql` builds one that carries a table. A generated table type holds one and carries one `rasql.Column` field per column, bound against whatever alias and namespace that table value holds, so application code names a column as a field the compiler checks rather than as a string, and never names a table type of its own. `Table.Ref()` reaches the `query.TableRef` underneath, and `Ref().Column(name)` is the one way to name a column the code only learns while it runs.
