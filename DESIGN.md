...
`rasql.Table[T]` is a struct holding one `query.TableRef` in an unexported field, so nothing outside `rasql` builds one that carries a table. A generated table type holds one and adds one accessor method per column, calling `Table.Column`. Application code then names a column as a method call rather than a string, and never names a table type of its own.
