...
func TestSourceIdentityAndDecoderValidation(t *testing.T) {
	...
	q := rasql.Select(ar, p).Where(rasql.EqualValue(rasql.Value(int64(1)), int64(1)))
	require.NoError(t, q.Validate())
	ac, err := rasql.BindColumn[int64, int64](ar, "id", "")
	require.NoError(t, err)
	columnProjection, err := rasql.Scalar("id", ac.Expr(), schema.IntegerType{}, "")
	require.NoError(t, err)
	columnQuery := rasql.Select(ar, columnProjection).Where(rasql.EqualValue(ac.Expr(), int64(1))).GroupBy(rasql.Group(ac.Expr())).OrderBy(rasql.AscExpr(ac.Expr()))
	require.NoError(t, columnQuery.Validate())
	bc, err := rasql.BindColumn[int64, int64](br, "id", "")
	require.NoError(t, err)
	outside := rasql.Select(ar, columnProjection).Where(rasql.EqualValue(bc.Expr(), int64(1)))
	var pe *rasql.PlanError
	require.ErrorAs(t, outside.Validate(), &pe)
	require.Equal(t, "invalid_source", pe.Code)
	joined := rasql.Select(ar, p).Join(br, rasql.EqualExpr(ac.Expr(), bc.Expr()))
	require.NoError(t, joined.Validate())
	duplicateSource, _ := b.As("same")
	duplicate := rasql.Select(ar, p).Join(duplicateSource, rasql.EqualValue(rasql.Value(int64(1)), int64(1)))
	...
	require.Error(t, rasql.Select(ar, projection2).Validate())
}
