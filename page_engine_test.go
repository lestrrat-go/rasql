...
func TestPageAfterEngines(t *testing.T) {
	t.Run("PostgreSQL pages match every explicit null order", func(t *testing.T) {
		...
		require.NoError(t, err)
		table, err := rasql.ViewOf[r5ComboRow](schema.TableDef{Name: tableName, Columns: []schema.ColumnDef{{Name: "id", Type: schema.IntegerType{}}, {Name: "rank", Type: schema.IntegerType{}, Nullable: true}}})
		require.NoError(t, err)
	...
	t.Run("MySQL refuses an explicit null order before taking a handle", func(t *testing.T) {
		...
		require.NoError(t, err)
		table, err := rasql.ViewOf[r5ComboRow](schema.TableDef{Name: tableName, Columns: []schema.ColumnDef{{Name: "id", Type: schema.IntegerType{}}, {Name: "sort_value", Type: schema.IntegerType{}, Nullable: true}}})
		require.NoError(t, err)
		relation, err := rasql.SourceOf(table, "p")
		require.NoError(t, err)
		id, err := rasql.BindColumn[r5ComboRow, int64](relation, "id", "")
	...
	t.Run("an autocommit mutation differs from an explicit transaction", func(t *testing.T) {
		...
		makeQuery := func(handle rasql.Handle) (rasql.Executor, rasql.Query[r5LiveRow], rasql.PageSpec[r5LiveRow]) {
			...
			table, tableErr := rasql.ViewOf[r5LiveRow](schema.TableDef{Name: tableName, Columns: []schema.ColumnDef{{Name: "id", Type: schema.IntegerType{}}}})
	...
	t.Run("live PostgreSQL 17 and MySQL 8.4", func(t *testing.T) {
		for _, tc := range []struct {
			...
			t.Run(tc.name, func(t *testing.T) {
				...
				require.NoError(t, err)
				table, err := rasql.ViewOf[r5LiveRow](schema.TableDef{Name: tableName, Columns: []schema.ColumnDef{{Name: "id", Type: schema.IntegerType{}}}})
				require.NoError(t, err)
				relation, err := rasql.SourceOf(table, "p")
				require.NoError(t, err)
