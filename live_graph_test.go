...
func testLiveGraph(t *testing.T, database *sql.DB, d dialect.Dialect) {
	...
	parentTable, err := rasql.TableOf[graphLiveParent](schema.TableDef{Name: parentsName, PrimaryKey: []string{"id"}, Columns: []schema.ColumnDef{{Name: "id", Type: schema.IntegerType{}}}})
	require.NoError(t, err)
