...
For each table it writes a `<table>_gen.go` file holding a row type with its mapping and scan methods, a table type with one column reference per column, the descriptor itself, and an accessor. This is the whole file for a `users` table of `id` and `email`, generated by the command above:
