...
Everything that stays the same from run to run lives in a checked-in `rasql.json` at the module root: the package name, the output directory, the dialect, the table selection, row-type names, static queries, and the pruning policy. The DSN is never read from it. See [the generator guide](docs/orm/01-codegen.md#the-settings-file).
...
Writing the same descriptors by hand works too. See [Schemas](docs/core/01-schema.md) and [`rasql codegen`](docs/orm/01-codegen.md).
