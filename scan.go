...
// conversions follow. [NullableScalar]'s decoder calls it for the one value a
// nullable scalar projection returns, and a hand-written ScanDestinations can
// call it for a destination that needs the same treatment.
func ScanValue[T any](destination *T, value any) error {
	if destination == nil {
		return fmt.Errorf("rasql: scan destination must not be nil")
	}
	if err := rowvalue.AssignValue(reflect.ValueOf(destination).Elem(), value); err != nil {
		return fmt.Errorf("rasql: scan value: %w", err)
	}
	return nil
}
