...
func TestValueSnapshot(t *testing.T) {
	t.Run("a value with a codec snapshots every supported input", func(t *testing.T) {
	...
	t.Run("a value with a codec rejects unsupported graphs", func(t *testing.T) {
	...
	t.Run("an equality carries snapshot errors into validation", func(t *testing.T) {
		type cycleNode struct{ Next *cycleNode }
		cycle := &cycleNode{}
		cycle.Next = cycle

		table, err := rasql.ViewOf[int64](schema.TableDef{Name: "values", Columns: []schema.ColumnDef{{Name: "id", Type: schema.IntegerType{}}}})
		require.NoError(t, err)
	...
	t.Run("the snapshotter runs recursively and wraps errors", func(t *testing.T) {
		calls := 0
	...
	t.Run("overlapping acyclic slices are allowed", func(t *testing.T) {
		base := []int{1, 2, 3}
		_, err := rasql.ValueWithCodec([][]int{base[:2], base[1:]}, "")
		require.NoError(t, err)
	})

	t.Run("pointer types at the same address are distinguished", func(t *testing.T) {
	...
	t.Run("the snapshotter runs once and builds a reusable query", func(t *testing.T) {
		calls := 0
	...
	t.Run("a snapshot detaches mutable containers", func(t *testing.T) {
		bytes := []byte("before")
