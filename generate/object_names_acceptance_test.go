...
func TestConsumer(t *testing.T) {
 ...
 table := generated.Customer(); insert, err := query.NewInsert(table.Ref(), query.Set(table.IDRef(), 1), query.Set(table.ScanRowColumnRef(), "scan"), query.Set(table.FooBarColumnRef(), "one"), query.Set(table.FooBarOtherColumnRef(), "two"), query.Set(table.JapaneseColumnRef(), "jp"), query.Set(table.ProfileNameColumnRef(), "profile"), query.Set(table.SayHiColumnRef(), "quote")); if err != nil { t.Fatal(err) }
...
func TestRelationships(t *testing.T) {
 ...
 userInsert, err := query.NewInsert(Users().Ref(), query.Set(Users().UserIDColumnRef(), 7), query.Set(Users().NameRef(), "user")); if err != nil { t.Fatal(err) }; if _, err = rasql.Exec(context.Background(), rdb, userInsert); err != nil { t.Fatal(err) }
 postInsert, err := query.NewInsert(Posts().Ref(), query.Set(Posts().PostIDColumnRef(), 8), query.Set(Posts().UserIDColumnRef(), 7), query.Set(Posts().BodyRef(), "post")); if err != nil { t.Fatal(err) }; if _, err = rasql.Exec(context.Background(), rdb, postInsert); err != nil { t.Fatal(err) }
