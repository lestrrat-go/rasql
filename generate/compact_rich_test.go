...
func TestRichCompactRuntime(t *testing.T) {
 ...
 create, err := store.Account().Create().ID(1).Nickname("Ada").Balance(domain.Money(7)).Plan(); if err != nil { t.Fatal(err) }; if _, err = rasql.Exec(ctx, executor, create); err != nil { t.Fatal(err) }
 source, err := store.Account().As("u"); if err != nil { t.Fatal(err) }; projection, err := store.AccountProjection(source); if err != nil { t.Fatal(err) }; query := rasql.Select(source, projection).OrderBy(rasql.AscExpr(source.ID.Expr())); columns := projection.Schema().Columns(); columns[0].Name = "caller_mutation"; if projection.Schema().Columns()[0].Name != "id" { t.Fatalf("projection schema was mutated: %#v", projection.Schema().Columns()) }
