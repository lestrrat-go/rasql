...
func TestRichCompactRuntime(t *testing.T) {
 ...
 profiled, err := rasql.AsExecutor(db, profile); if err != nil { t.Fatal(err) }
 codecs, err := rasql.NewCodecRegistry(map[rasql.CodecID]rasql.ValueCodec{"money": moneyCodec{}}); if err != nil { t.Fatal(err) }; executor, err := rasql.WithCodecs(profiled, codecs); if err != nil { t.Fatal(err) }
