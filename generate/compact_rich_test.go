...
func TestRichCompactRuntime(t *testing.T) {
 ...
 rootSource, err := store.Account().As("u3"); if err != nil { t.Fatal(err) }; rootExpressions, err := (store.AccountColumns{}).Bind(rootSource); if err != nil { t.Fatal(err) }; rootProjection, err := store.AccountProjection(rootExpressions); if err != nil { t.Fatal(err) }; rootQuery := rasql.Select(rootSource, rootProjection)
 junctionSource, err := store.Membership().As("ur"); if err != nil { t.Fatal(err) }; junctionExpressions, err := (store.MembershipColumns{}).Bind(junctionSource); if err != nil { t.Fatal(err) }
