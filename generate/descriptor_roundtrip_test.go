...
// schemaPackagePath scopes the reflection walk in requireFixturesCoverEveryField
// to types declared in package schema, so a field of a foreign type (string,
// bool, a slice or map of those) is treated as a leaf rather than walked
// into.
...
//
// reflect.Type cannot discover the concrete implementations of the
// schema.ColumnType interface, so they are added as extra roots alongside
...
func requireFixturesCoverEveryField(t *testing.T, tables ...schema.TableDef) {
	...
	for _, table := range tables {
		walkFieldValue(reflect.ValueOf(table), seen)
	}
...
// registered separately; see requireFixturesCoverEveryField). visited
...
// walkFieldValue marks, into seen, "<Type>.<Field>" for every exported field
// of a package-schema struct anywhere in value that holds a non-zero value,
// recursing through pointers, interfaces, slices, arrays, and map values to
// reach nested structs -- including a schema.ColumnType interface field's
...
func walkFieldValue(value reflect.Value, seen map[string]bool) {
	...
	case reflect.Ptr, reflect.Interface:
		...
		walkFieldValue(value.Elem(), seen)
	case reflect.Slice, reflect.Array:
		for i := 0; i < value.Len(); i++ {
			walkFieldValue(value.Index(i), seen)
		}
	case reflect.Map:
		for _, key := range value.MapKeys() {
			walkFieldValue(value.MapIndex(key), seen)
		}
	case reflect.Struct:
		t := value.Type()
		for i := 0; i < t.NumField(); i++ {
			...
			if field.PkgPath == "" && t.PkgPath() == schemaPackagePath && !fieldValue.IsZero() {
				seen[t.String()+"."+field.Name] = true
			}
			walkFieldValue(fieldValue, seen)
		}
	}
}
...
// package into. It is not part of what rasqlgen generates: it exists only
// so a structural comparison can happen with both values as real Go values
// inside the process that will run it, rather than crossing a process
// boundary through a serialization format that could itself mask a field.
// It is placed in the same package specifically so it can reach each
// table's unexported literal variable (accessor.varName) directly and
// compare it, with reflect.DeepEqual, against what calling the exported
// accessor (accessor.funcName) returns -- the same comparison an earlier
// version of this test made in the parent process after decoding JSON the
// child wrote, which is what let ExclusionConstraints's inconsistent
// json:",omitempty" tag silently repair the one round-trip defect a JSON
// hop could mask (see the commit message this test's history carries for
// that finding).
func writeRoundTripCheckFile(t *testing.T, outputDir string, accessors []descriptorAccessor) {
	t.Helper()

	var source strings.Builder
	source.WriteString("package store\n\n")
	source.WriteString("import (\n\t\"fmt\"\n\t\"reflect\"\n\t\"strings\"\n\n\t\"github.com/lestrrat-go/rasql/schema\"\n)\n\n")
	source.WriteString("// RunDescriptorRoundTripCheck reports \"PASS\" when calling every generated\n")
	source.WriteString("// descriptor accessor in this package returns a value reflect.DeepEqual to\n")
	source.WriteString("// the literal descriptor rasqlgen rendered it from, or a \"FAIL\" report naming\n")
	source.WriteString("// which table did not survive otherwise.\n")
	source.WriteString("func RunDescriptorRoundTripCheck() string {\n")
	source.WriteString("\tvar failures []string\n")
	source.WriteString("\tfor _, check := range []struct {\n\t\tname string\n\t\twant schema.TableDef\n\t\tgot  schema.TableDef\n\t}{\n")
	for _, accessor := range accessors {
		source.WriteString("\t\t{" + strconv.Quote(accessor.tableName) + ", " + accessor.varName + ", " + accessor.funcName + "()},\n")
	}
	source.WriteString("\t} {\n")
	source.WriteString("\t\tif !reflect.DeepEqual(check.want, check.got) {\n")
	source.WriteString("\t\t\tfailures = append(failures, fmt.Sprintf(\"%s: accessor result is not reflect.DeepEqual to the rendered literal\\n  literal:  %#v\\n  accessor: %#v\", check.name, check.want, check.got))\n")
	source.WriteString("\t\t}\n\t}\n")
	source.WriteString("\tif len(failures) == 0 {\n\t\treturn \"PASS\"\n\t}\n")
	source.WriteString("\treturn \"FAIL\\n\" + strings.Join(failures, \"\\n\")\n")
	source.WriteString("}\n")

	require.NoError(t, os.WriteFile(filepath.Join(outputDir, "descriptor_round_trip_check.go"), []byte(source.String()), 0o600))
}
...
// generated accessor.
//
// Two checks run inside that child process rather than by shipping any
// value back to this one, so nothing about either check depends on a
// serialization format that could itself mask a field the way JSON's
// ExclusionConstraints json:",omitempty" tag once did:
//
//   - The child renders the descriptors it read back through
...
//     stdout. This function requires it byte-for-byte identical to the
//     first rendering.
//   - The child also calls store.RunDescriptorRoundTripCheck(), a
//     hand-written companion file writeRoundTripCheckFile placed beside the
//     generated files, which reflect.DeepEqual-compares each table's
//     accessor result against the unexported literal variable it was
//     rendered from -- both real Go values inside that same process -- and
//     writes "PASS" or a "FAIL" report to its own stderr. This function
//     requires that to read "PASS".
func roundTripDescriptors(t *testing.T, tables ...schema.TableDef) {
	...
	writeRoundTripCheckFile(t, outputDir, accessors)
	...
	require.Equal(t, "PASS", checkResult,
		"the in-process structural comparison inside the scratch module's own child process failed")
}
