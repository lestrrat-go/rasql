...
func TestConsumer(t *testing.T) {
  ...
  rdb, err := rasql.Open(t.Context(), db, dialect.SQLite()); if err != nil { t.Fatal(err) }
  executor := rasql.Executor(rdb)
  ...
  update, err := Update(3); if err != nil { t.Fatal(err) }; outcome, err := rasql.Exec(t.Context(), executor, update); if err != nil || outcome.Affected != 0 { t.Fatalf("exec 0: %#v %v", outcome, err) }
  update, err = Update(2); if err != nil { t.Fatal(err) }; outcome, err = rasql.Exec(t.Context(), executor, update); if err != nil || outcome.Affected != 1 { t.Fatalf("exec 1: %#v %v", outcome, err) }
  update, err = Update(1); if err != nil { t.Fatal(err) }; outcome, err = rasql.Exec(t.Context(), executor, update); if err != nil || outcome.Affected != 2 { t.Fatalf("exec 2: %#v %v", outcome, err) }
  cancelled, cancel := context.WithCancel(t.Context()); cancel(); update, err = Update(0); if err != nil { t.Fatal(err) }; _, err = rasql.Exec(cancelled, executor, update); if !errors.Is(err, context.Canceled) { t.Fatalf("cancel: %v", err) }
  if err = rasql.Within(t.Context(), executor, nil, func(ctx context.Context, tx rasql.Executor) error { plan, e := Update(2); if e != nil { return e }; _, e = rasql.Exec(ctx, tx, plan); return e }); err != nil { t.Fatalf("transaction: %v", err) }
