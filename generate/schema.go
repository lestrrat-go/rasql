...
// PackageSource returns the typed surface for every table in packageName: the
// row types, table types, column accessors and relationships. It is the
// multi-table form of TableSource and, like TableSource, never emits a
// table's runtime descriptor, so its output alone does not compile; pair it
// with DescriptorSource.
...
// function, As, and any relationships. It never emits a table's runtime
// descriptor; DescriptorSource owns that.
...
//
// The emitted literal is the merged definition prepareSchema produces, not
// the input one: prepareSchema appends a derived belongs-to relationship for
// every foreign key that no explicit relationship matches, so the literal
// can carry relationships the caller's own input did not state.
