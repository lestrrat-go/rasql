...
func TestSchemaIsDeterministicAndCompiles(t *testing.T) {
	...
	require.Contains(t, string(source), "var ordersTable = rasql.MustTable[OrdersRow](schema.Table{")
	require.Contains(t, string(source), "var usersTable = rasql.MustTable[UsersRow](schema.Table{")
	require.Contains(t, string(source), "func Orders() rasql.Table[OrdersRow] {")
