...
New generator examples should use the owned-program workflow: `rasqlgen init` creates `gen/main.go`, the program calls `catalog.FromDatabase` and `generate.Store`, and `go generate ./...` followed by `go run ./gen -check` refreshes and verifies the package. The `schemasource` and `bootstrapsource` examples remain compatibility fixtures because their purpose is to document the direct commands and their existing semantics.
...
The Taskboard module still uses the compatibility `rasqlmigrate` plus `rasqlgen schema -dsn` directive. It is a separate migration target, not the template for a new application's generator; a follow-up can move it to an owned `gen/main.go` without changing this repository's generated-file checks.
