...
Name a column in an example through the generated columns struct, as `columns, err := (store.UsersColumns{}).Bind(users)` followed by `columns.ID`, and give a hand-written table the wrapper type and the accessor methods that do the same job for it. Either form is checked by the compiler, so a renamed column breaks the build, while `Table.Ref().Column("id")` is checked only once the statement is assembled. An example spelling the column as a string shows the reader the weaker of two calls they can both make.
