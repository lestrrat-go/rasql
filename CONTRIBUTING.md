...
Both jobs reach a live test only by expanding package patterns, so a test file in a directory the go tool passes over runs in neither of them. `go help packages` names those directories: any whose name begins with `.` or `_`, plus `testdata`, and anything under `vendor`. `TestIntegrationJobListsEveryDBTestGuardedPackage` cannot rescue such a file either, since no package list it could ask for would reach it -- put live tests in ordinary package directories.
