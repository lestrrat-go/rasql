...
Name a column in an example through an accessor method, as `users.ID()`, and give a hand-written table the wrapper type and the accessor methods that make that possible. The accessor is checked by the compiler, so a renamed column breaks the build, while `Table.Column("id")` is checked only once the statement is assembled. An example spelling the column as a string shows the reader the weaker of two calls they can both make.
