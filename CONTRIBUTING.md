...
Name a column in an example through an accessor method, as `users.ID()`, and give a hand-declared table the wrapper type and the accessor methods that make that possible. `examples/query_example_tables_test.go` declares the shared `users` and `orders` tables that way, and `examples/rasql_self_join_example_test.go` declares one beside the example that uses it. The reason is what the reader takes away: the accessor is checked by the compiler, so a renamed column breaks the build, while `Table.Column("id")` is checked only once the statement is assembled. Most readers arrive holding a generated store, where the accessors already exist, so an example spelling the column as a string shows them the weaker of two calls they can both make.
