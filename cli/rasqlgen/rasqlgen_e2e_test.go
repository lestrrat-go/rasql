...
// append a replace back to this checkout, so it needs no "go mod tidy" and
// runs offline), runs "init" through "go run" exactly as a real consumer
// would, generates a store from a real SQLite database through the
// scaffold's own "go generate ./..." directive, and drives "go run ./gen
// -check" through both the clean and the stale state.
func TestGoRunInitScaffoldGenerates(t *testing.T) {
	...
	// imports; nothing more needs adding for this test to build offline.
