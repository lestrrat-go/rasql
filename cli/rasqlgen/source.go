...
func schemaSourceProgram(importPath, packageName, outputDir string, tableNames []string) []byte {
	...
	// This is the throwaway main.go this command writes, runs, and deletes;
	// it is never marker-checked, so it deliberately keeps the pre-split
	// marker rather than gaining a fourth family of its own.
