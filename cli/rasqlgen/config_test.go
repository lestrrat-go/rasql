...
func TestLoadConfigCanonicalSettings(t *testing.T) {
  ...
  "tables": {"exclude": ["audit_log"], "row_names": {"users": "User"}}
	...
	require.Equal(t, "sqlite", settings.Dialect)
	require.Equal(t, "migrations", settings.Migrations)
...
func TestLoadConfigRefusals(t *testing.T) {
	testCases := []struct {
		...
		{name: "legacy engine key", settings: `{"engine":{"dialect":"sqlite","profile":"sqlite-3.35"},"package":"store","output":"internal/store"}`, expected: `unknown field "engine"`},
		{name: "legacy schema key", settings: `{"schema":{"kind":"migrations","identity":"app","paths":["migrations/*.sql"]},"package":"store","output":"internal/store"}`, expected: `unknown field "schema"`},
	}
