...
func TestConfigNamesUsesExactObjectIdentity(t *testing.T) {
	settings := config{Tables: configTables{Names: map[string]configObjectNames{
	...
	require.Equal(t, configObjectNames{Accessor: "BillingEvents", TableType: "BillingEventsTable", RowType: "BillingEventsRow", FileBase: "billing_events"}, names[schema.ObjectName{Schema: "billing", Name: "events"}])
