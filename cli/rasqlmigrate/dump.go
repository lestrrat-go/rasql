...
// PostgreSQL identity column whose sequence departs from bigint's own
// defaults, a MySQL default rasql cannot re-emit as SQL, and a
// foreign-key cycle all refuse the run by name rather than being written
// wrong or dropped. A PostgreSQL identity column with a default sequence
// and a MySQL AUTO_INCREMENT column both dump and replay normally: render
// now emits the DDL for both.
//
...
// returns it. Identity and the five IdentityStart..IdentityCycle sequence
// facts are meaningful for PostgreSQL only, and AutoIncrement for MySQL
// only; each dialect's fetch function leaves the other dialect's fields
// zero.
type dumpColumnFact struct {
	...
	// IdentityStart, IdentityIncrement, IdentityMinimum, IdentityMaximum,
	// and IdentityCycle are Identity's own sequence facts, read from
	// information_schema.columns.identity_start /
	// identity_increment / identity_minimum / identity_maximum /
	// identity_cycle. They are nullable because a non-identity column
	// reports NULL for all five. render emits a bare
	// GENERATED ... AS IDENTITY / GENERATED BY DEFAULT AS IDENTITY clause
	// with no START WITH or INCREMENT BY, so a sequence other than
	// bigint's own default (start 1, increment 1, minimum 1, maximum
	// 9223372036854775807, cycle NO) would replay with a different
	// sequence than the source database's, silently: hasDefaultIdentitySequence
	// is what applyDumpGuards uses to refuse that case by name instead.
	//
	// Not carried at all, by this dump or by a BIGSERIAL column's dump
	// today: the sequence's CACHE setting (pg_sequence.seqcache, not
	// exposed through information_schema) and the sequence's current
	// value.
...
// match bigint's own defaults: start 1, increment 1, minimum 1, maximum
// 9223372036854775807, cycle NO. Those are the only values render's bare
// GENERATED ... AS IDENTITY clause reproduces; any other sequence would be
// silently dropped on replay.
func (f dumpColumnFact) hasDefaultIdentitySequence() bool {
	return f.IdentityStart.String == "1" &&
		f.IdentityIncrement.String == "1" &&
		f.IdentityMinimum.String == "1" &&
		f.IdentityMaximum.String == "9223372036854775807" &&
...
// column type gate needs and the five identity sequence facts
// hasDefaultIdentitySequence checks, rather than adding further round
// trips for them.
