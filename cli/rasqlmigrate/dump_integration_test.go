...
func TestDumpPostgreSQLRefusesIdentityColumn(t *testing.T) {
	ctx := t.Context()
	source := dbtest.PostgreSQLDB(t)
	...
	require.ErrorContains(t, err, `table "teams" column "id" is an identity column`)
}
...
func TestDumpMySQLRefusesAutoIncrementColumn(t *testing.T) {
	ctx := t.Context()
	...
	require.ErrorContains(t, err, `table "teams" column "id" is AUTO_INCREMENT`)
}
