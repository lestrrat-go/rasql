...
func Example_rasql_partial_update() {
	...
	// The query package takes a query.ColumnRef, and Ref().Column is the
	// generated table's only way to produce one.
	// SQL: UPDATE users SET email = ? WHERE users.id < ? (arguments: "ada@example.com", 100)
	statement, err := query.NewUpdate(users.Ref(), query.Set(users.Ref().Column("email"), "ada@example.com"))
	if err != nil {
		fmt.Printf("failed to build update: %s\n", err)
		return
	}
	statement, err = statement.WithWhere(query.LessThan(users.Ref().Column("id"), 100))
	if err != nil {
	...
	kept, err := query.NewSelect(users.Ref(), users.Ref().Column("id"), users.Ref().Column("email"))
	if err != nil {
		fmt.Printf("failed to build select: %s\n", err)
		return
	}
	kept, err = kept.WithWhere(query.Equal(users.Ref().Column("id"), 512))
	if err != nil {
