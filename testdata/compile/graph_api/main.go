...
func main() {
	users := rasql.MustTableOf[userRow](schema.TableDef{Name: "users", Columns: []schema.ColumnDef{{Name: "id", Type: schema.IntegerType{}}}})
	tasks := rasql.MustTableOf[taskRow](schema.TableDef{Name: "tasks", Columns: []schema.ColumnDef{{Name: "id", Type: schema.IntegerType{}}, {Name: "user_id", Type: schema.IntegerType{}}}})
	labels := rasql.MustTableOf[labelRow](schema.TableDef{Name: "labels", Columns: []schema.ColumnDef{{Name: "id", Type: schema.IntegerType{}}}})
	links := rasql.MustTableOf[linkRow](schema.TableDef{Name: "user_labels", Columns: []schema.ColumnDef{{Name: "user_id", Type: schema.IntegerType{}}, {Name: "label_id", Type: schema.IntegerType{}}}})
	employees := rasql.MustTableOf[employeeRow](schema.TableDef{Name: "employees", Columns: []schema.ColumnDef{{Name: "id", Type: schema.IntegerType{}}, {Name: "manager_id", Type: schema.IntegerType{}}}})
	ur, _ := users.As("u")
	tr, _ := tasks.As("t")
	lr, _ := labels.As("l")
	jr, _ := links.As("j")
	...
	up := rasql.Select(ur, mustProjection([]rasql.ProjectionItem{rasql.Item("id", uid.Expr(), schema.IntegerType{}, "")}, userDecoder{schema: intSchema("id")}))
	tp := rasql.Select(tr, mustProjection([]rasql.ProjectionItem{rasql.Item("id", tid.Expr(), schema.IntegerType{}, ""), rasql.Item("user_id", tuid.Expr(), schema.IntegerType{}, "")}, taskDecoder{schema: intSchema("id", "user_id")}))
	lp := rasql.Select(lr, mustProjection([]rasql.ProjectionItem{rasql.Item("id", lid.Expr(), schema.IntegerType{}, "")}, labelDecoder{schema: intSchema("id")}))
	_ = rasql.Select(jr, mustProjection([]rasql.ProjectionItem{rasql.Item("user_id", juid.Expr(), schema.IntegerType{}, ""), rasql.Item("label_id", jlid.Expr(), schema.IntegerType{}, "")}, linkDecoder{schema: intSchema("user_id", "label_id")}))
	ep := rasql.Select(er, mustProjection([]rasql.ProjectionItem{rasql.Item("id", eid.Expr(), schema.IntegerType{}, ""), rasql.Item("manager_id", emanager.Expr(), schema.IntegerType{}, "")}, employeeDecoder{schema: intSchema("id", "manager_id")}))
	...
	labelsEdge, _ := rasql.ManyThrough("labels", uk, ju, jl, lk, jr, labelsPlan, rasql.EdgeOptions{}, func(g *userGraph, v rasql.LoadedMany[labelGraph]) { g.Labels = v })
