...
func TestGeneratedProfile(t *testing.T) {
	...
	require.NoError(t, err)
	projectProjection, err := store.ProjectsProjection(projects)
	require.NoError(t, err)
	projectQuery := rasql.Select(projects, projectProjection).Where(rasql.EqualValue(projects.ID.Expr(), int64(1))).OrderBy(rasql.AscExpr(projects.ID.Expr()))
	...
	require.NoError(t, err)
	taskProjection, err := store.TasksProjection(tasks)
	require.NoError(t, err)
	memberProjection, err := store.MembersProjection(members)
	require.NoError(t, err)
	taskQuery := rasql.Select(tasks, taskProjection).OrderBy(rasql.AscExpr(tasks.ID.Expr()))
	memberQuery := rasql.Select(members, memberProjection).OrderBy(rasql.AscExpr(members.ID.Expr()))
	...
	require.NoError(t, err)
	projectTasks, err := store.ProjectsTasksEdge(projects, tasks, tasksPlan, rasql.EdgeOptions{PerParentLimit: 5, Order: []rasql.OrderTerm{rasql.AscExpr(tasks.ID.Expr())}}, func(parent *profileGraph, loaded rasql.LoadedMany[profileTask]) { parent.Tasks = loaded })
	require.NoError(t, err)
	...
	if %t {
		...
		createProjection, projectionErr := store.MembersProjection(createSource)
	...
	patchPlan, err := store.Members().Patch().Name("patched").Where(rasql.EqualValue(patchSource.ID.Expr(), int64(4001))).Plan()
	require.NoError(t, err)
	if %t {
		patchProjection, projectionErr := store.MembersProjection(patchSource)
		require.NoError(t, projectionErr)
