//go:build unix

package rowvalue_test

import (
	"database/sql"
	"reflect"
	"testing"

	"github.com/lestrrat-go/rasql/internal/dbtest"
	"github.com/lestrrat-go/rasql/internal/rowvalue"
	"github.com/stretchr/testify/require"
)

// TestSumOverIntegerColumnDecodesAgainstLiveDatabases pins the engine fact the
// text-encoded integer path exists for. MySQL computes SUM() over an INTEGER
// column as DECIMAL and its driver delivers a DECIMAL as bytes, so the value
// reaching rasql is []byte("350") and not int64(350). PostgreSQL computes the
// same sum as bigint and delivers int64.
//
// Every other test of this path builds the driver value by hand, which cannot
// confirm what either server actually sends. This one reads the value off a
// live connection first, asserts its Go type, and only then decodes it, so a
// server that changes what it sends fails here rather than silently widening
// what the decoder has to accept.
func TestSumOverIntegerColumnDecodesAgainstLiveDatabases(t *testing.T) {
	for _, test := range []struct {
		name string
		open func(*testing.T) *sql.DB
		// delivered is the Go type the engine's driver hands back for the sum.
		// The two engines disagree, which is the whole point of the test.
		delivered reflect.Type
	}{
		{
			name:      "postgresql",
			open:      dbtest.PostgreSQLDB,
			delivered: reflect.TypeFor[int64](),
		},
		{
			name:      "mysql",
			open:      dbtest.MySQLDB,
			delivered: reflect.TypeFor[[]byte](),
		},
	} {
		t.Run(test.name, func(t *testing.T) {
			ctx := t.Context()
			database := test.open(t)

			_, err := database.ExecContext(ctx, "CREATE TABLE totals (amount INTEGER NOT NULL)")
			require.NoError(t, err, "create table")
			_, err = database.ExecContext(ctx, "INSERT INTO totals (amount) VALUES (100), (250)")
			require.NoError(t, err, "seed rows")

			var delivered any
			require.NoError(t,
				database.QueryRowContext(ctx, "SELECT SUM(amount) FROM totals").Scan(&delivered),
				"read the sum",
			)
			require.Equal(t, test.delivered, reflect.TypeOf(delivered),
				"the driver value the engine delivers for SUM() over an integer column",
			)

			row, err := rowvalue.NewRow([]string{"total"}, []any{delivered})
			require.NoError(t, err)

			var total int64
			require.NoError(t, rowvalue.Assign(row, "total", &total), "decode the sum")
			require.Equal(t, int64(350), total)
		})
	}
}
