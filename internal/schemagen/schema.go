...
func schemaSource(packageName string, tables, allTables []schema.TableDef, descriptors descriptorMode) ([]byte, error) {
	bindingState := newBindingState(tables)
	...
	if len(tables) > 0 {
		...
		for _, imported := range bindingState.imports {
	...
	for _, table := range tables {
		writeRowType(&source, table, bindingState)
		source.WriteString("\n")
...
func writeRowType(source *bytes.Buffer, table schema.TableDef, bindings bindingState) {
	...
	for _, column := range table.Columns {
		...
		source.WriteByte(' ')
		source.WriteString(bindings.typeFor(column, column.Nullable))
		source.WriteString("\n")
	}
	source.WriteString("}\n")
}
