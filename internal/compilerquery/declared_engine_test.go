...
func TestAnalyzeSQLitePreparesRealQuery(t *testing.T) {
	db := sqliteUsersDB(t)
	result := analyzeDeclaredEngine(t, db, "sqlite", "sqlite-3.35", "SELECT id, name FROM users WHERE id = {{bind \"id\" users.id}}", "id", "integer", "id", "integer")
...
func TestAnalyzeMySQLPreparesLiveQuery(t *testing.T) {
	...
	result := analyzeDeclaredEngine(t, db, "mysql", "mysql-8.4", "SELECT id, name FROM `"+table+"` WHERE id = {{bind \"id\" "+table+".id}}", "id", "integer", "id", "integer")
	require.Len(t, result.Queries, 1)
	require.Equal(t, compilerir.CertaintyDeclared, result.Queries[0].Parameters[0].TypeCertainty)
	require.Equal(t, compilerir.CertaintyDeclared, result.Queries[0].Results[1].TypeCertainty)
}

func analyzeDeclaredEngine(t *testing.T, db *sql.DB, engine, profileID string, sqlText, parameterName, parameterScalar, resultName, resultScalar string) schemasource.AnalysisResult {
	t.Helper()
	root := t.TempDir()
	require.NoError(t, os.WriteFile(filepath.Join(root, "q.sql"), []byte(sqlText), 0o600))
	nullable := false
	a, err := compilerquery.NewAnalyzer(compilerquery.Config{ModuleRoot: root, Queries: []compilerquery.QueryConfig{{ID: "q", Input: "q.sql", Engine: engine, Function: "Q", Operation: "select", Cardinality: "many", Parameters: []compilerquery.ValueDeclaration{{Name: parameterName, Scalar: parameterScalar, Nullable: &nullable}}, Results: []compilerquery.ValueDeclaration{{Name: resultName, Scalar: resultScalar, Nullable: &nullable}, {Name: "name", Scalar: "text", Nullable: &nullable}}}}})
	require.NoError(t, err)
