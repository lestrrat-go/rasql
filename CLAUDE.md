...
An example names a column through a generated-shape accessor method, never as a string. A reader copies what the examples do, and almost every reader has generated a store, where `users.ID()` is the call the compiler checks and a renamed column becomes a build failure. `Table.Column("id")` gives that check up, so an example that reaches for it teaches the weaker form as if it were the normal one. A table an example declares by hand gets the wrapper type and one accessor method per column, the way `examples/query_example_tables_test.go` and `examples/rasql_self_join_example_test.go` write it.
