...
Generated source a page shows is checked in and regenerated by the same flag: `examples/store/users_gen.go`, `examples/store/schema_gen.go`, `examples/store/schema_gen_test.go`, `examples/store/user_by_email_gen.go`, `examples/bootstrapsource/internal/tables/users_gen.go`, `examples/bootstrapsource/internal/tables/tables_gen.go`, and `examples/bootstrapsource/internal/tables/hints.go`. NEVER add a JSON schema snapshot to generate one from — this repository generates from a Go table definition or from migrations applied to a throwaway database, and 29dc9e5 removed the last snapshot.
