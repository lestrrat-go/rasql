...
The Go code that talks to those tables is generated from a checked-in schema
snapshot and query manifest. `rasql schema update` captures a disposable
database once; `rasql generate` then works offline from the lock and manifest.
[`rasql generate`](../../../docs/orm/01-codegen.md) describes the command and
its settings file.
...
Chapter 3 captures that settled shape into a checked-in migration. Chapter 4
locks the schema and generates the store offline. Chapter 5 builds a typed
repository and graph page, and chapter 6 adds the view model and HTTP layer.
