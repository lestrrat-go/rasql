...
The Go code that talks to those tables is generated by reading the database
itself. `rasql codegen generate -dsn` applies the migration directory and then
reads the catalog it produces; `rasql codegen check` proves the checked-in
result still matches its inputs, with a database or without one.
[`rasql codegen`](../../../docs/orm/01-codegen.md) describes the command and
its settings file.
...
generates the store from the database. Chapter 5 builds a typed repository
and graph page, and chapter 6 adds the view model and HTTP layer.
