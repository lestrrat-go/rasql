...
// table level and inside each element type -- clones the result, and requires
// nothing to be shared. A container field added to TableDef or to any
// descriptor type it reaches fails here as soon as that type's own Clone
...
// container an element owns is filled too. A field of any other kind is left
// at its zero value: a slice and a map are what a plain assignment leaves a
// descriptor and its clone sharing.
...
// requireContainerFieldsUnshared reports a failure for each slice or map field
// source and clone still share, naming the field's own path through the
// descriptor. It descends into slice elements the same way fillContainerFields
// does, so a container an element owns is checked too.
...
// including a new key in each map, so anything a clone still shares with its
// source carries the write across.
