...
// IdentityGeneration names whether an identity column always draws its
// value from its sequence or only when no value is given explicitly. Its
// zero value, the empty string, means the column is not an identity column
// at all, which is what every ColumnDef and every checked-in generated
// file written before this field existed has always meant.
//
// An identity column is a different feature from a generated column
// (ColumnDef.GeneratedExpression / GeneratedStorage): a generated column
// computes its value from other columns of the same row and can never be
// written to, while an identity column draws its value from a sequence
// and, in the IdentityByDefault form, still accepts an explicit value on
// INSERT.
//
// PostgreSQL produces both IdentityAlways and IdentityByDefault. MySQL
// produces only IdentityByDefault, from an AUTO_INCREMENT column, since
// MySQL has no form that rejects an explicit value. SQLite has no
// per-column identity feature and never produces either value.
type IdentityGeneration string

const (
	// IdentityAlways marks a column whose value is always assigned from
	// its sequence: an explicit value on INSERT is rejected. PostgreSQL's
	// GENERATED ALWAYS AS IDENTITY produces this value; MySQL has no
	// equivalent form.
	IdentityAlways IdentityGeneration = "ALWAYS"
	// IdentityByDefault marks a column whose sequence supplies a value
	// only when INSERT omits one: an explicit value is accepted and kept.
	// PostgreSQL's GENERATED BY DEFAULT AS IDENTITY and MySQL's
	// AUTO_INCREMENT both produce this value.
	IdentityByDefault IdentityGeneration = "BY DEFAULT"
)

func (g IdentityGeneration) valid() bool {
	switch g {
