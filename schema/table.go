...
type ColumnDef struct {
	...
	// Identity names whether this column is an identity column, and if so,
	// which of the two generations it is. Its zero value, the empty
	// string, means the column is not an identity column, which is what
	// every ColumnDef and every checked-in generated file written before
	// this field existed has always meant.
	//
	// Identity is a different feature from GeneratedExpression /
	// GeneratedStorage: a computed generated column derives its value from
	// other columns of the same row and can never be written, while an
	// identity column draws its value from a sequence and, in the
	// IdentityByDefault form, still accepts an explicit value on INSERT.
	// Table.Validate rejects a column that states both.
	//
	// PostgreSQL produces both IdentityAlways and IdentityByDefault, from
	// a GENERATED ALWAYS AS IDENTITY or GENERATED BY DEFAULT AS IDENTITY
	// column. MySQL produces only IdentityByDefault, from an
	// AUTO_INCREMENT column. SQLite never produces either value: it has no
	// per-column identity feature. See IdentityGeneration's own doc for
	// what each value means for rendering and for the typed write path.
	Identity IdentityGeneration `json:",omitempty"`
...
type IndexDef struct {
	...
	// written before this field existed has always meant.
	//
	// render.CreateIndexes renders a non-empty Predicate verbatim and
	// untranslated on a dialect with dialect.CapabilityPartialIndex
	// (PostgreSQL and SQLite), and refuses on a dialect without it (MySQL,
	// which has no partial-index concept and so never produces a
	// non-empty Predicate either). Predicate carries no record of which
...
func (t TableDef) Validate() error {
	...
	for i, column := range t.Columns {
		...
		if !column.Identity.valid() {
			return validationError(path+".identity", "unsupported identity generation %q", column.Identity)
		}
		if column.Identity != "" && column.GeneratedExpression != "" {
			return validationError(path+".identity", "an identity column must not also state a GeneratedExpression")
		}
		if column.Identity != "" && column.Default != "" {
			return validationError(path+".identity", "an identity column must not also state a Default")
		}
		if column.Identity != "" && column.Nullable {
			return validationError(path+".identity", "an identity column must not be Nullable")
		}
