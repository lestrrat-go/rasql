...
func TestPlanRoundTripAndArgumentIsolation(t *testing.T) {
	...
	operation, err := changeplan.NewOperation("create", changeplan.OperationCreateTable, nil, []changeplan.ObjectID{future.ID()}, nil, nil, changeplan.Digest{1}, []stmt.Statement{stmt.New(sqltext.Text("CREATE TABLE created (id INTEGER)"))}, changeplan.TransactionRequired, false, []stmt.Statement{})
	require.NoError(t, err)
	backfill, err := changeplan.NewOperation("backfill", changeplan.OperationBackfill, []changeplan.OperationID{"create"}, []changeplan.ObjectID{future.ID()}, nil, nil, changeplan.Digest{2}, []stmt.Statement{statement}, changeplan.TransactionForbidden, false, nil)
	require.NoError(t, err)
...
func TestStableTopologicalOrderUsesOriginalOrder(t *testing.T) {
	...
	require.NoError(t, err)
	first, err := changeplan.NewOperation("first", changeplan.OperationNativeSQL, nil, []changeplan.ObjectID{"starting"}, nil, nil, changeplan.Digest{1}, []stmt.Statement{stmt.New(sqltext.Text("SELECT 1"))}, changeplan.TransactionForbidden, false, nil)
	require.NoError(t, err)
	second, err := changeplan.NewOperation("second", changeplan.OperationNativeSQL, nil, []changeplan.ObjectID{"starting"}, nil, nil, changeplan.Digest{2}, []stmt.Statement{stmt.New(sqltext.Text("SELECT 2"))}, changeplan.TransactionForbidden, false, nil)
	require.NoError(t, err)
