...
func TestCatalogCopiesDefinitionsAndSupportsResolvedSteps(t *testing.T) {
	...
	require.NoError(t, err)
	operation, err := changeplan.NewOperation("alter", changeplan.OperationAddColumn, []changeplan.OperationID{}, []changeplan.ObjectID{id}, nil, []changeplan.Fact{fact}, resultDigest, []stmt.Statement{stmt.New(sqltext.Text("ALTER TABLE tasks ADD COLUMN title TEXT"))}, changeplan.TransactionEngineDefault, false, []stmt.Statement{})
	require.NoError(t, err)
