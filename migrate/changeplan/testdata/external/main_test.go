...
func TestPublicProfileAndLockBoundary(t *testing.T) {
	...
	require.NoError(t, err)
	operation, err := changeplan.NewOperation("add-title", changeplan.OperationAddColumn, []changeplan.OperationID{}, []changeplan.ObjectID{startingID}, []changeplan.Fact{precondition}, []changeplan.Fact{postcondition}, []stmt.Statement{stmt.New(sqltext.Text(`ALTER TABLE "main"."tasks" ADD COLUMN "title" TEXT NOT NULL`))}, changeplan.TransactionEngineDefault, false, []stmt.Statement{})
	require.NoError(t, err)
