...
func TestNativeTypeSQLiteDeclarationMatrix(t *testing.T) {
	first, err := sql.Open("sqlite", filepath.Join(t.TempDir(), "first.db"))
	require.NoError(t, err)
	t.Cleanup(func() { _ = first.Close() })
	_, err = first.ExecContext(t.Context(), `CREATE TABLE declarations (integer_col INTEGER, int2_col INT2, int8_col INT8, empty_col, blob_col BLOB DEFAULT X'01')`)
	require.NoError(t, err)
	inspector, err := inspect.New(first, dialect.SQLite())
	require.NoError(t, err)
	table, err := inspector.Table(t.Context(), "declarations")
	require.NoError(t, err)
	require.Nil(t, table.Columns[0].NativeType)
	require.Equal(t, &schema.NativeTypeDef{Dialect: "sqlite", Name: "INT2", Kind: schema.NativeOther}, table.Columns[1].NativeType)
	require.Equal(t, &schema.NativeTypeDef{Dialect: "sqlite", Name: "INT8", Kind: schema.NativeOther}, table.Columns[2].NativeType)
	require.Nil(t, table.Columns[3].NativeType)
	require.Nil(t, table.Columns[4].NativeType)
	rendered, err := render.CreateTable(dialect.SQLite(), table)
	require.NoError(t, err)
	second, err := sql.Open("sqlite", filepath.Join(t.TempDir(), "second.db"))
	require.NoError(t, err)
	...
	secondInspector, err := inspect.New(second, dialect.SQLite())
	require.NoError(t, err)
	roundTrip, err := secondInspector.Table(t.Context(), "declarations")
	require.NoError(t, err)
	require.Equal(t, table.Columns, roundTrip.Columns)
}
