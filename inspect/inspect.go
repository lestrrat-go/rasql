...
// an error: this package never returns a schema.TableDef silently missing columns.
...
func (i Inspector) informationSchemaTable(ctx context.Context, namespace, tableName string) (schema.TableDef, error) {
	...
	table := schema.TableDef{Schema: namespace, Name: tableName, Columns: columns, PrimaryKey: primaryKey}
...
func (i Inspector) sqliteTable(ctx context.Context, databaseName string, tableName string) (schema.TableDef, error) {
	if databaseName != "" && databaseName != "main" && !sqliteRetainedQueryer(i.queryer) {
		return schema.TableDef{}, sqliteRetainedHandleError()
	}
	queryer := i.queryer
	if database, ok := queryer.(*sql.DB); ok {
		connection, err := database.Conn(ctx)
		if err != nil {
			return schema.TableDef{}, fmt.Errorf("inspect: acquire SQLite connection: %w", err)
		}
	...
	return inspector.sqliteTableOnConnection(ctx, databaseName, tableName)
}

func (i Inspector) sqliteTableOnConnection(ctx context.Context, databaseName string, tableName string) (schema.TableDef, error) {
	...
	if options.columnCount >= 0 && metadataRows != options.columnCount {
		return schema.TableDef{}, &IncompleteMetadataError{Table: tableName, Visible: int(metadataRows), Actual: int(options.columnCount)}
	}
	if len(metadata) == 0 {
	...
	columns := make([]schema.ColumnDef, 0, len(metadata))
	for _, column := range metadata {
	...
	if definition != nil {
		...
		return schema.TableDef{}, fmt.Errorf("inspect: SQLite table %q cannot be represented: UNIQUE constraint definitions are unavailable", tableName)
	}
...
func (i Inspector) sqliteTableNamesOnConnection(ctx context.Context, databaseName string) ([]TableName, error) {
	...
	// PRAGMA table_list is unavailable before SQLite 3.37; sqlite_master keeps those engines supported.
	legacyRefs, err := i.sqliteLegacyTableNames(ctx, databaseName)
	if err != nil {
		if tableListErr != nil {
...
func sqliteChecks(statement *sqlitequery.CreateTableStatement, tableName string) ([]schema.CheckDef, error) {
	...
	for _, column := range statement.Columns {
		for _, constraint := range column.Constraints {
			...
			checks = append(checks, schema.CheckDef{Name: sqliteIdentifierName(constraint.Name), Expression: expression})
		}
	}
	for _, constraint := range statement.Constraints {
		if constraint.Kind != sqlitequery.ConstraintCheck {
...
func (i Inspector) sqliteIndexes(ctx context.Context, databaseName, tableName string) ([]schema.IndexDef, []schema.UniqueDef, error) {
	...
	for rows.Next() {
		...
		columns, expressions, keys, err := i.sqliteIndexKeys(ctx, databaseName, name, definition)
		if err != nil {
			return nil, nil, err
		}
		index := schema.IndexDef{Name: name, Columns: columns, Expressions: expressions, Keys: keys, Unique: unique}
		if partial {
...
// sqliteIndexKey accumulates one key's facts across the single index_xinfo
// row that describes it: its text (a bare column name or, for an
// expression key, the expression text sqliteIndexKeys resolves separately),
// whether it is an expression, and the two per-key facts SQLite can attach
// beyond that — DESC order and a non-default collation.
...
// returns them as exactly one of Columns, Expressions, or Keys, matching
// schema.IndexDef's own three-way convention: Keys when at least one key is
// ordered DESC or names a non-default collation, since neither fact fits a
// flat string list; otherwise Expressions when at least one key is not a
// plain column, since index_xinfo reports a NULL column name for an
// expression key and rasql-sqlite's parsed CREATE INDEX definition is the
// only place that key's actual expression text lives; otherwise Columns.
...
func (i Inspector) sqliteIndexKeys(ctx context.Context, databaseName, indexName string, definition *sqlitequery.CreateIndexStatement) ([]string, []sqltext.Text, []schema.IndexKeyDef, error) {
	...
	rows, err := i.queryer.QueryContext(ctx, query)
	if err != nil {
		return nil, nil, nil, fmt.Errorf("inspect: read SQLite index %q columns: %w", indexName, err)
	}
	defer func() { _ = rows.Close() }()

	keys := make([]sqliteIndexKey, 0)
	needsKeyDetail := false
	keyPosition := 0
	for rows.Next() {
		...
		if err := rows.Scan(&sequence, &tableColumn, &name, &descending, &collation, &keyColumn); err != nil {
			return nil, nil, nil, fmt.Errorf("inspect: scan SQLite index %q column: %w", indexName, err)
		}
		if keyColumn == 0 {
			continue
		}
		key := sqliteIndexKey{descending: descending != 0}
		...
		if keyPosition >= len(definition.Elements) {
			return nil, nil, nil, fmt.Errorf("inspect: SQLite index %q cannot be represented: its CREATE INDEX definition has fewer keys than reported", indexName)
		}
	...
	if err := rows.Err(); err != nil {
		return nil, nil, nil, fmt.Errorf("inspect: iterate SQLite index %q columns: %w", indexName, err)
	}
	if len(keys) == 0 {
		return nil, nil, nil, fmt.Errorf("inspect: SQLite index %q has no columns", indexName)
	}
	if needsKeyDetail {
...
func (i Inspector) readUniqueConstraints(ctx context.Context, query string, arguments ...any) ([]schema.UniqueDef, error) {
	...
	for rows.Next() {
		...
		var temporal bool
		var storageParameters sql.NullString
		var tablespace sql.NullString
		var replicaIdentity bool
		var collation sql.NullString
		if err := rows.Scan(&name, &column, &deferrable, &initiallyDeferred, &nullsNotDistinct, &includeColumns, &temporal, &storageParameters, &tablespace, &replicaIdentity, &collation); err != nil {
		...
		if len(constraints) == 0 || constraints[len(constraints)-1].Name != name {
			constraints = append(constraints, schema.UniqueDef{
				...
				Temporal:          temporal,
				StorageParameters: splitPostgreSQLStorageParameters(storageParameters),
		...
		if collation.Valid {
			if constraint.Collations == nil {
				constraint.Collations = make(map[string]string, 1)
			}
			constraint.Collations[column] = collation.String
		}
	}
	if err := rows.Err(); err != nil {
...
// constraints, one row per element, grouped by constraint name the same way
...
func (i Inspector) readExclusionConstraints(ctx context.Context, query string, arguments ...any) ([]schema.ExclusionDef, error) {
	...
	var exclusions []schema.ExclusionDef
	for rows.Next() {
		var name string
		var method string
		var expression string
		...
		var deferrable bool
		var initiallyDeferred bool
		if err := rows.Scan(&name, &method, &expression, &operator, &predicate, &deferrable, &initiallyDeferred); err != nil {
			return nil, fmt.Errorf("inspect: scan exclusion constraint: %w", err)
		}
		if len(exclusions) == 0 || exclusions[len(exclusions)-1].Name != name {
			exclusionMethod := schema.IndexMethod("")
			if method != "btree" {
				exclusionMethod = schema.IndexMethod(method)
			}
...
// buildingIndexKey accumulates one key's facts across the several rows
// readIndexes scans for its index, one row per key: text holds a bare
// column name for a plain key or the key's expression text for an
// expression key, and the remaining fields are the per-key facts that
// force the accumulated keys into def.Keys instead of def.Columns or
// def.Expressions — see buildingIndex's own doc.
type buildingIndexKey struct {
	text          string
	isExpression  bool
	descending    bool
	collation     string
	...
	prefixLength  int
	nullsOrder    schema.NullsOrder
}

// buildingIndex accumulates one index's keys across the several rows
// readIndexes scans for it, one row per key. Once every row for the index
// has been seen, the accumulated keys are assigned to def.Keys, when at
// least one of them carries a per-key fact beyond a plain ascending
// expression, or otherwise to def.Columns or def.Expressions, matching
// schema.IndexDef's own three-way convention.
...
func (i Inspector) readIndexes(ctx context.Context, query string, mysqlIndexHasExpression bool, arguments ...any) ([]schema.IndexDef, error) {
	...
	for rows.Next() {
		...
		if i.dialect.Name() == "mysql" {
			...
			if !strings.EqualFold(indexType, "BTREE") {
				method = schema.IndexMethod(indexType)
			}
			invisible = !bool(visible)
			key.descending = strings.EqualFold(collation.String, "D")
			if prefixLength.Valid {
				key.prefixLength = int(prefixLength.Int64)
			}
			switch {
			case expression.Valid:
				key.text = expression.String
				key.isExpression = true
			case nullableColumn.Valid:
				key.text = nullableColumn.String
			default:
			...
			var indexNullsNotDistinct bool
			var indexKeyNullsFirst bool
			if err := rows.Scan(&name, &unique, &attributeName, &keyExpression, &accessMethod, &indexPredicate, &descending, &operatorClass, &collation, &indexIncludeColumns, &indexNotValid, &indexStorageParameters, &indexTablespace, &indexReplicaIdentity, &indexNullsNotDistinct, &indexKeyNullsFirst); err != nil {
				return nil, fmt.Errorf("inspect: scan index: %w", err)
			}
			...
			nullsNotDistinct = indexNullsNotDistinct
			key.descending = descending
			if operatorClass.Valid {
				key.operatorClass = operatorClass.String
			}
			if collation.Valid {
				key.collation = collation.String
			}
			// A key's NULLS FIRST/NULLS LAST placement is recorded only
			// when it overrides the default its own ASC/DESC direction
			// already implies: NULLS LAST for ASC, NULLS FIRST for DESC.
			switch {
			case descending && !indexKeyNullsFirst:
				key.nullsOrder = schema.NullsLast
			case !descending && indexKeyNullsFirst:
				key.nullsOrder = schema.NullsFirst
			}
			switch {
			case attributeName.Valid:
				key.text = attributeName.String
			case keyExpression.Valid:
				key.text = keyExpression.String
				key.isExpression = true
			default:
	...
	for index, building := range indexes {
		...
		for _, key := range building.keys {
			if key.descending || key.collation != "" || key.operatorClass != "" || key.prefixLength != 0 || key.nullsOrder != "" {
				needsKeyDetail = true
			}
			if key.isExpression {
				hasExpression = true
...
func (i Inspector) readForeignKeys(ctx context.Context, query string, arguments ...any) ([]schema.ForeignKeyDef, error) {
	...
	for rows.Next() {
		...
		var deferrable bool
		var initiallyDeferred bool
		var deleteSetColumns sql.NullString
		var validated bool
		var enforced bool
		var temporal bool
		...
		if len(keys) == 0 || keys[len(keys)-1].Name != name {
			keys = append(keys, schema.ForeignKeyDef{
				...
				Temporal:         temporal,
				DeleteSetColumns: splitPostgreSQLIncludedColumns(deleteSetColumns),
			})
		}
		key := &keys[len(keys)-1]
		if key.ReferencedSchema != referencedSchema || key.ReferencedTable != referencedTable || key.Match != match || key.OnDelete != onDelete || key.OnUpdate != onUpdate || key.Deferrable != deferrability || key.NotValid != notValid || key.NotEnforced != notEnforced || key.Temporal != temporal {
...
func postgreSQLInformationQueries(version int) informationQueries {
	...
	if version >= postgreSQL16Version {
