...
func (i Inspector) rejectUnsupportedIndexes(ctx context.Context, query string, argument any) error {
	...
	return fmt.Errorf("inspect: index %q cannot be represented: rasql supports only valid, non-partial B-tree indexes with simple ascending columns, no included columns, default operator classes and collations, default persistent storage options and tablespaces, and distinct nulls", name)
}
