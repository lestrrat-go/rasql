...
func (i Inspector) readUniqueConstraints(ctx context.Context, query string, argument any) ([]schema.UniqueConstraint, error) {
	...
	for rows.Next() {
		...
		if unsupportedIndexMetadata {
			return nil, fmt.Errorf("inspect: unique constraint %q cannot be represented: rasql does not support unique constraints whose backing indexes have nondefault storage options or tablespaces, or are selected as replica identity", name)
		}
